...
MODEL = "ensemble"
Log_Start_Time = '2016-12-12 03:00:00'
...
PROJECTS = {
    "tiramisu": {
        ...
        "db": "postgresql",
        "db_name": "tiramisu",
        "schema": "../combinedTiramisuSchema.sql",
        "workload": "tiramisu-out.log",
        "original_dir": "../tiramisu-combined-csv",
        # "predict_dirs": ["../prediction-results/agg-60/horizon-4320/ensemble"],
        "predict_dirs": ["../prediction-results/agg-10/horizon-10/hybrid"],
        "cluster_assignment": "../cluster-coverage/None-0.8-assignments.pickle",
        "cluster_coverage": "../cluster-coverage/coverage.pickle",
        "cluster_num": 3,
        "predict_aggregate": 10,  # time interval
        "trace_aggregate": 10,
        "index_duration": 5,
        "warmup_period": 40,
    ...
    "admission": {
        ...
        "cluster_num": 3,
        "predict_aggregate": 10, # Seconds
        "trace_aggregate": 10, # Seconds
        "index_duration": 5, # Minutes
...
def unpackLine(line):
    # 用逗号分割有问题，因为插入语句里面有很多逗号
    line_split = line.strip().split(",")
    line_split = [eval(x) for x in line_split]
    # print(line_split)
    line_split[0] = line_split[0][:-8]
...
def executeQuery(config, dbconfig, batch):
    ...
    while ((time.time() < timeout) and (curr_q < len(batch) - 1)):
        try:
            ...
            # print('aaaaaaaaaaaaaaaaannnnnnnnnnnnnnnnnnnnnnnddddddddddddwwwwwwwww', res)
...
def processLine(line):
    time_stamp = datetime.strptime(line[0][:-8], "%Y-%m-%d %H:%M:%S")
    time_stamp =  time_stamp.replace(second=0, microsecond=0)
    conn_id = line[1]
...
def runTrace(config, dbconfig, cnx, cursor):
    ...
    with open(config["workload"], "r", encoding='utf-16') as f:
        reader = csv.reader(f)
        for line in reader:
            time_stamp, conn_id, query = processLine(line)
            if query == False:
                continue
            if time_stamp > datetime.strptime(Log_Start_Time, "%Y-%m-%d %H:%M:%S"):
                file_lines.append((time_stamp, conn_id, query))
    f.close()
    ...
    if not EXPLAIN:
        result_file = "results/" + "forecast-" + str(config['predict_aggregate']) + ".csv"
    else:
    ...
    if STATIC_SUGGEST:
        result_file = "results/" + "static-" + str(config['predict_aggregate']) + ".csv"
    ...
    for i in range(NUM_ITERS):
        ...
        with open(result_file, "w+") as file:
            ...
            for j in range(0, NUM_MINUTES, config["trace_aggregate"]):
                ...
                # batch为当前时刻往后10分钟以内的logs，file_lines为去掉该部分剩余的logs
